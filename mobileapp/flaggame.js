var　FLAG_NUM = 5; //旗の数
var BALL_PIXEL_PER_UPDATE = -5; //ボールをイベントごとに移動させる距離
...
$(document).on("pageinit", "#p-top", function(e) {
	...
	var FIELD_Y_LIMIT = $("#field").height() - BALL_SIZE;//yの限界値
	var ballX = FIELD_X_LIMIT / 2;
	var ballY = FIELD_Y_LIMIT / 2;
	var startTime = 0;
	var timerContinue = false;
	var onDeviceMotion = function(e) {
		//画面の傾きによって、加速度センサーの値を補正してボールを動かす
	...
	for ( var i = 1; i <= FLAG_NUM; i++) {
		...
		//経過時間を10msecごとに画面に表示する。
		(function timer() {
			$("#elapsed").html((new Date().getTime() - startTime) / 1000);
			if (timerContinue) {
				setTimeout(timer, 10);
			}
		})();
